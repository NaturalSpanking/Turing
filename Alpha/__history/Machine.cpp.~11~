...
{
	rule errRule;
	char tempChar = GetTapeChar(GetTapePosition());
...
rule Machine::GetTableRule(unsigned state, char c)
{
	rule errRule;
...
int Machine::SetTableRule(unsigned state, char c, rule r)
{
...
int Machine::SetTapeChar(unsigned index, char c)
{
...
char Machine::GetTapeChar(unsigned index)
{
